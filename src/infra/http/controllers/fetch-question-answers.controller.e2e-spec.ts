...
describe('Fetch question answers (e2e)', () => {
  ...
  test('[GET] /questions/:questionId/answers', async () => {
    ...
    const [firstAnswer, secondAnswer] = await Promise.all([
      answerFactory.makePrismaAnswer({ questionId, authorId }),
      answerFactory.makePrismaAnswer({ questionId, authorId }),
    ])
    ...
    expect(response.body).toEqual({
      answers: expect.arrayContaining([
        expect.objectContaining({ content: firstAnswer.content }),
        expect.objectContaining({ content: secondAnswer.content }),
      ]),
    })
  })
})
